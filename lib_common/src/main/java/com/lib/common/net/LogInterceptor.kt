...
import com.johnson.injoi.base.log.LogCat
import okhttp3.*
import okio.Buffer
...
class LogInterceptor(block: (LogInterceptor.() -> Unit)? = null) : Interceptor {
    ...
    private fun logRequest(request: Request, connection: Connection?) {
        ...
        sb.appendln("Https Request Start")
        when (logLevel) {
            LogLevel.NONE -> {
                /*do nothing*/
            }
    ...
    private fun logResponse(response: Response) {
        val sb = StringBuffer()
        sb.appendln("Https Respone Start")
        when (logLevel) {
            LogLevel.NONE -> {
                /*do nothing*/
            }
        ...
        if (logLevel == LogLevel.BODY) {
            ...
            kotlin.runCatching {
                ...
                LogCat.json(peekBody.string(), logTag)
            }.getOrNull()
        }
    }
    ...
    private fun logIt(any: Any, tempLevel: ColorLevel? = null) {
        when (tempLevel ?: colorLevel) {
            ColorLevel.VERBOSE -> LogCat.v(any.toString(), logTag)
            ColorLevel.DEBUG -> LogCat.d(any.toString(), logTag)
            ColorLevel.INFO -> LogCat.i(any.toString(), logTag)
            ColorLevel.WARN -> LogCat.w(any.toString(), logTag)
            ColorLevel.ERROR -> LogCat.e(any.toString(), logTag)
        }
    }
